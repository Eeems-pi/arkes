...
def command(args: Namespace) -> None:
    ...
    def render_build(job_id: str) -> list[str]:
        ...
        lines = [
            ...
            "    builder: ${{ needs.builder_tag.outputs.tag }}",
        ]
        if job_id != "rootfs":
            lines += [
    ...
    sections = [
        ...
        indent(
            [
                ...
                "",
                "builder_tag:",
                "  name: Transform builder tag",
                "  needs: builder",
                "  runs-on: ubuntu-latest",
                "  outputs:",
                "    tag: ${{ (github.event_name != 'pull_request' || github.event.pull_request.head.repo.id == github.event.pull_request.base.repo.id) && needs.builder.outputs.unique_tag || 'master' }}",
                "  steps:",
                "    - run: true",
                "",
                "tool-niricfg:",
                ...
                "    packages: read",
                "  container:",
                f"    image: {BUILDER}:${{{{ needs.builder_tag.outputs.tag }}}}",
                "    options: >-",
                "      --privileged",
                ...
                "      with:",
                "        path: .venv",
                "        key: venv-${{ needs.builder_tag.outputs.tag }}-${{ hashFiles('make/check.py') }}",
                "    - name: Cache go",
                "      uses: actions/cache@v5",
                "      with:",
                "        path: |",
